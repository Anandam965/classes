...
def speak_interview_question(question, auto_reload_seconds=None):
    safe_question = json.dumps(str(question))
    reload_js = f"setTimeout(() => window.parent.location.reload(), {int(auto_reload_seconds) * 1000});" if auto_reload_seconds else ""
...
def admin_ai_feedback_notes():
    ...
    with tabs[0]:
        ...
        if st.button("Create Communication Exam", key="create_communication_exam") and exam_title.strip():
            try:
                supabase.table("communication_exams").insert({"title": exam_title.strip(), "enabled": True}).execute()
...
def user_dashboard(preview_mode=False):
    ...
    if not preview_mode:
        try:
            if str(st.query_params.get("comm_exam", "")) == "1":
