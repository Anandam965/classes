...
def show_ai_mock_interview(user_id):
    ...
    try:
        questions = supabase.table("ai_interview_questions").select("*").eq("active", True).order("id").execute().data or []
    except Exception as e:
    ...
    if st.session_state.get("interview_spoken_for") != spoken_key:
        ...
        speak_interview_question(current.get("question", ""), auto_reload_seconds=remaining + 1)
...
def admin_ai_feedback_notes():
    ...
    with tabs[0]:
        with st.form("add_interview_question", clear_on_submit=True):
            ...
            if st.form_submit_button("Add Interview Question", type="primary") and question.strip():
                try:
                    supabase.table("ai_interview_questions").insert({
                        "question": question.strip(), "expected_answer": expected_answer.strip(),
