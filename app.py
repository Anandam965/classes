...
def speak_interview_question(question):
...
def show_ai_mock_interview(user_id):
    ...
    if not questions:
        ...
        return
    idx = int(st.session_state.get("interview_question_index", 0)) % len(questions)
    current = questions[idx]
    st.caption(f"Question {idx + 1} of {len(questions)} is being asked by voice. Listen, then record your answer.")
    ...
    if audio:
        ...
        if st.button("Save Answer & Next", type="primary", use_container_width=True):
            try:
                ...
                st.session_state.interview_question_index = (idx + 1) % len(questions)
    ...
    with tabs[0]:
        with st.form("add_interview_question", clear_on_submit=True):
            ...
            if st.form_submit_button("Add Interview Question", type="primary") and question.strip():
                try:
                    supabase.table("ai_interview_questions").insert({"question": question.strip(), "active": True}).execute()
        ...
        try:
            ...
            for q in interview_questions:
                ...
                with c1: st.write(q.get("question"))
                with c2:
