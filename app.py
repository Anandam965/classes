...
def load_exam_questions_fast(exam_id):
    """Avoid fetching the same aptitude questions again on every Streamlit rerun."""
    return supabase.table("questions").select("*").eq("exam_id", exam_id).execute().data or []


def render_exam_detail_view(exam):
    st.markdown(f"### {exam.get('title', 'Exam')}")
    questions = load_exam_questions_fast(str(exam["id"]))
...
create table if not exists ai_interview_questions (
  ...
  ai_evaluate boolean default false,
  grammar_weight numeric default 40,
  content_weight numeric default 60,
  active boolean default true,
  created_at timestamptz default now()
);
create table if not exists communication_exams (
  id bigint generated by default as identity primary key,
  title text not null,
  instructions text,
...
alter table communication_exams add column if not exists instructions text;
create table if not exists communication_exam_sections (
  id bigint generated by default as identity primary key,
  communication_exam_id bigint not null references communication_exams(id) on delete cascade,
  start_question integer not null,
  end_question integer not null,
  instructions text not null,
  created_at timestamptz default now(),
  check (start_question >= 1 and end_question >= start_question)
);
alter table ai_interview_questions add column if not exists communication_exam_id bigint references communication_exams(id) on delete cascade;
...
alter table ai_interview_questions add column if not exists expected_answer text;
alter table ai_interview_questions add column if not exists time_seconds integer not null default 60;
alter table ai_interview_questions add column if not exists max_marks numeric not null default 10;
alter table ai_interview_questions add column if not exists ai_evaluate boolean default false;
alter table ai_interview_questions add column if not exists grammar_weight numeric default 40;
alter table ai_interview_questions add column if not exists content_weight numeric default 60;
alter table ai_interview_responses add column if not exists score numeric;
alter table ai_interview_responses add column if not exists evaluation text;
alter table ai_interview_responses add column if not exists evaluated_by_ai boolean default false;
create table if not exists feedback_settings (
  id integer primary key default 1 check (id = 1),
  collection_enabled boolean default false,
  updated_at timestamptz default now()
);
insert into feedback_settings (id, collection_enabled) values (1, false) on conflict (id) do nothing;
...
-- Create a private Storage bucket named interview-recordings in Supabase Storage.
'''


def feedback_collection_enabled():
    try:
        rows = supabase.table("feedback_settings").select("collection_enabled").eq("id", 1).execute().data or []
        return bool(rows and rows[0].get("collection_enabled"))
    except Exception:
        return False


def show_feedback_page(user_id):
    st.subheader("Feedback")
    if not feedback_collection_enabled():
        st.info("Feedback collection is not open right now.")
        return
    st.caption("Admin feedback collect cheyyamani request chesaru. Mee suggestion submit cheyyandi.")
    with st.form("user_feedback_form", clear_on_submit=True):
        message = st.text_area("Your feedback", max_chars=2000, height=150)
        if st.form_submit_button("Send Feedback", type="primary"):
            if not message.strip():
                st.warning("Feedback enter cheyyandi.")
            else:
                try:
                    supabase.table("user_feedback").insert({"user_id": user_id, "feedback": message.strip()}).execute()
                    st.success("Feedback admin ki send ayyindi.")
                except Exception as e:
...
def show_notes_page():
    ...
    try:
        ...
        st.error(f"Notes load avvaledu. Admin SQL setup run cheyyali: {e}")
        return
    if not notes:
        st.info("Notes inka add cheyyaledu.")
        return
    topics = {}
    for note in notes:
        topics.setdefault(str(note.get("topic") or "General"), []).append(note)
    for topic, entries in topics.items():
        with st.expander(f"📁 {topic} ({len(entries)})"):
            for number, note in enumerate(entries, 1):
                st.markdown(f"**{number}. {note.get('question', '')}**")
...
def speak_interview_question(question, finish_phase=""):
    safe_question = json.dumps(str(question))
    finish_js = f'''utterance.onend = () => {{
          const url = new URL(window.parent.location.href);
          url.searchParams.set('comm_phase', '{finish_phase}');
          window.parent.location.assign(url.toString());
        }};''' if finish_phase else ""
    components.html(f'''<script>
      try {{
        const utterance = new SpeechSynthesisUtterance({safe_question});
        utterance.lang = 'en-IN'; utterance.rate = 0.92;
        {finish_js}
...
def evaluate_interview_audio(audio, question):
    if not question.get("ai_evaluate") or not st.secrets.get("GEMINI_API_KEY", ""):
        return None, "Manual review", False
    temp_path = ""
    try:
        suffix = ".wav" if "wav" in str(audio.type or "") else ".webm"
        with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as tmp:
            tmp.write(audio.getvalue()); temp_path = tmp.name
        uploaded = genai.upload_file(temp_path, mime_type=audio.type or "audio/webm")
        max_marks = float(question.get("max_marks") or 10)
        prompt = f'''Return only JSON: {{"score": number, "feedback": string}}. Evaluate the spoken answer.
Question: {question.get("question", "")}
Expected answer: {question.get("expected_answer", "")}
Maximum marks: {max_marks}. Grammar weight: {question.get("grammar_weight", 40)}%. Content weight: {question.get("content_weight", 60)}%.'''
        result = genai.GenerativeModel("gemini-1.5-flash").generate_content([prompt, uploaded])
        data = json.loads(str(result.text or "").replace("```json", "").replace("```", "").strip())
        return max(0, min(max_marks, float(data.get("score", 0)))), str(data.get("feedback") or "AI evaluated."), True
    except Exception as e:
        return None, f"AI evaluation pending/manual review: {e}", False
    finally:
        if temp_path:
...
def render_communication_timer(remaining, next_question_index):
    """Browser timer changes the question index in the URL, so a page refresh cannot reset to Q1."""
    components.html(f'''<style>
      .comm-timer {{ width:112px;height:112px;border-radius:50%;display:grid;place-items:center;
        margin:5px auto;background:conic-gradient(#16a34a calc(var(--p) * 1%), #e5e7eb 0); }}
      .comm-timer>div {{width:88px;height:88px;border-radius:50%;display:grid;place-items:center;background:#fff;
        font:700 25px Arial;color:#111827;}}
    </style><div class="comm-timer" id="commTimer"><div id="commNumber">{int(remaining)}</div></div>
    <script>
      let remaining = {int(remaining)}, total = Math.max(1, {int(remaining)});
      const draw = () => {{
        document.getElementById('commNumber').textContent = remaining;
        document.getElementById('commTimer').style.setProperty('--p', (remaining / total) * 100);
      }}; draw();
      const tick = setInterval(() => {{
        ...
        if (remaining <= 0) {{
          clearInterval(tick);
          const url = new URL(window.parent.location.href);
          url.searchParams.set('comm_exam', '1');
          url.searchParams.set('comm_question', '{int(next_question_index)}');
          url.searchParams.set('comm_phase', 'ask');
          window.parent.location.assign(url.toString());
        }}
      }}, 1000);
    </script>''', height=125)
...
def show_ai_mock_interview(user_id):
    st.subheader("Communication Round")
    try:
        url_exam_active = str(st.query_params.get("comm_exam", "")) == "1"
        url_question = int(st.query_params.get("comm_question", "0"))
        url_exam_id = str(st.query_params.get("comm_exam_id", ""))
        url_phase = str(st.query_params.get("comm_phase", "ask"))
        if url_exam_active:
            st.session_state.communication_round_started = True
            st.session_state.interview_question_index = url_question
            if url_exam_id: st.session_state.communication_exam_id = url_exam_id
            if url_phase == "record" and st.session_state.get("interview_recording_phase_for") != url_question:
                st.session_state.interview_question_started_at = time.time()
                st.session_state.interview_recording_phase_for = url_question
    except Exception:
        pass
    try:
        communication_exams = supabase.table("communication_exams").select("*").eq("enabled", True).order("id").execute().data or []
    except Exception:
        communication_exams = []
    selected_exam_id = str(st.session_state.get("communication_exam_id") or "")
    if not selected_exam_id and not st.session_state.get("communication_round_started"):
        if not communication_exams:
            st.info("Admin first Communication Exam create cheyyali.")
            return
        st.caption("Select a Communication Exam. Each exam has its own question set.")
        for exam in communication_exams:
            if st.button(f"Open: {exam.get('title')}", key=f"open_comm_exam_{exam['id']}", use_container_width=True):
                st.session_state.communication_exam_id = str(exam["id"])
                try:
                    st.query_params["comm_exam_id"] = str(exam["id"])
                    st.query_params["comm_phase"] = ""
                    if "comm_section_done" in st.query_params: del st.query_params["comm_section_done"]
                except Exception: pass
                st.rerun()
        return
    selected_exam = next((e for e in communication_exams if str(e.get("id")) == selected_exam_id), {})
    if not st.session_state.get("communication_round_started") and selected_exam.get("instructions"):
        st.markdown("### Instructions")
        st.write(selected_exam.get("instructions"))
        ...
        return
    try:
        query = supabase.table("ai_interview_questions").select("*").eq("active", True)
        if selected_exam_id:
            query = query.eq("communication_exam_id", selected_exam_id)
        questions = query.order("id").execute().data or []
    except Exception as e:
        st.error(f"Interview questions load avvaledu. Admin SQL setup run cheyyali: {e}")
        return
    if not questions:
        st.info("Admin interview questions add chesaka ikkada voice interview start avuthundi.")
        return
    try:
        exam_sections = supabase.table("communication_exam_sections").select("*").eq("communication_exam_id", selected_exam_id).order("start_question").execute().data or []
    except Exception:
        exam_sections = []
    if not st.session_state.get("communication_round_started"):
        st.caption("Questions screen lo display avvavu; voice lo vastayi. Start click chesina ventane round begin avuthundi.")
        if st.button("Start Communication Round", type="primary", use_container_width=True):
            st.session_state.communication_round_started = True
            st.session_state.interview_question_index = 0
            try:
                st.query_params["comm_exam"] = "1"
            ...
            st.rerun()
        return
    idx = int(st.session_state.get("interview_question_index", 0))
    if idx >= len(questions):
        st.success("Communication round completed.")
        st.session_state.communication_round_started = False
        try:
            del st.query_params["comm_exam"]
            del st.query_params["comm_question"]
            del st.query_params["comm_exam_id"]
            del st.query_params["comm_phase"]
            if "comm_section_done" in st.query_params: del st.query_params["comm_section_done"]
        except Exception: pass
        st.session_state.communication_exam_id = ""
        return
    current = questions[idx]
    phase = str(st.query_params.get("comm_phase", "ask"))
    section = next((s for s in exam_sections if int(s.get("start_question") or 0) == idx + 1), None)
    if section:
        section_done_phase = f"section_{section['id']}_read"
        section_already_done = str(st.query_params.get("comm_section_done", "")) == str(section["id"])
        if not section_already_done and phase != section_done_phase:
            st.markdown(f"### Instructions: Questions {section.get('start_question')}–{section.get('end_question')}")
        ...
        if not section_already_done and st.button("Next", key=f"section_next_{section['id']}", type="primary", use_container_width=True):
            try:
                st.query_params["comm_phase"] = "ask"
                st.query_params["comm_section_done"] = str(section["id"])
            except Exception: pass
            st.rerun()
        ...
        if st.button("Save Answer & Next", type="primary", use_container_width=True):
            try:
                ...
                supabase.storage.from_("interview-recordings").upload(
                    path, audio.getvalue(), {"content-type": audio.type or "audio/webm"}
                )
                score, evaluation, ai_done = evaluate_interview_audio(audio, current)
                supabase.table("ai_interview_responses").insert({
...
def admin_ai_feedback_notes():
    ...
    with st.expander("Database setup SQL", expanded=False):
        st.code(get_ai_features_sql(), language="sql")
    tabs = st.tabs(["Interview Questions", "Feedback", "Notes"])
    with tabs[0]:
        st.markdown("#### Communication Exams")
        exam_title = st.text_input("New Communication Exam title", key="new_communication_exam")
        exam_instructions = st.text_area("Instructions (optional — user ki display and voice lo chadavutayi)", key="new_communication_exam_instructions")
        if st.button("Create Communication Exam", key="create_communication_exam") and exam_title.strip():
            try:
                supabase.table("communication_exams").insert({"title": exam_title.strip(), "instructions": exam_instructions.strip() or None, "enabled": True}).execute()
                st.success("Communication exam created."); st.rerun()
            except Exception as e: st.error(str(e))
        try:
            communication_exams = supabase.table("communication_exams").select("*").eq("enabled", True).order("id").execute().data or []
        except Exception:
            communication_exams = []
        exam_options = {str(e.get("title")): e.get("id") for e in communication_exams}
        if not exam_options:
            st.info("First create a Communication Exam, then add its question set.")
        with st.expander("Add instructions for a question range (example: Questions 1–10)"):
            with st.form("add_communication_section", clear_on_submit=True):
                section_exam = st.selectbox("Communication Exam", list(exam_options.keys()) or ["Create an exam first"], key="section_exam")
                s_col1, s_col2 = st.columns(2)
                with s_col1: start_question = st.number_input("Start question", min_value=1, value=1, step=1)
                with s_col2: end_question = st.number_input("End question", min_value=1, value=10, step=1)
                section_instructions = st.text_area("Instructions for this question range")
                if st.form_submit_button("Save Section Instructions", type="primary"):
                    if section_exam in exam_options and section_instructions.strip() and end_question >= start_question:
                        try:
                            supabase.table("communication_exam_sections").insert({
                                "communication_exam_id": exam_options[section_exam],
                                "start_question": int(start_question), "end_question": int(end_question),
                                "instructions": section_instructions.strip(),
                            }).execute()
                            st.success("Section instructions saved."); st.rerun()
                        except Exception as e: st.error(str(e))
        with st.form("add_interview_question", clear_on_submit=True):
            selected_exam_name = st.selectbox("Communication Exam", list(exam_options.keys()) or ["Create an exam first"])
            question = st.text_area("Question (student ki voice lo adagabadutundi)")
            expected_answer = st.text_area("Expected answer / key points")
            c_time, c_marks = st.columns(2)
            with c_time: time_seconds = st.number_input("Time for this question (seconds)", min_value=10, max_value=600, value=60)
            with c_marks: max_marks = st.number_input("Maximum marks", min_value=1.0, max_value=100.0, value=10.0, step=1.0)
            use_ai = st.checkbox("Let AI evaluate this answer", value=False)
            c_grammar, c_content = st.columns(2)
            with c_grammar: grammar_weight = st.number_input("Grammar weight (%)", min_value=0, max_value=100, value=40)
            with c_content: content_weight = st.number_input("Content weight (%)", min_value=0, max_value=100, value=60)
            if st.form_submit_button("Add Interview Question", type="primary") and question.strip() and selected_exam_name in exam_options:
                try:
                    supabase.table("ai_interview_questions").insert({
                        "communication_exam_id": exam_options[selected_exam_name], "question": question.strip(), "expected_answer": expected_answer.strip(),
                        "time_seconds": int(time_seconds), "max_marks": float(max_marks),
                        "ai_evaluate": use_ai, "grammar_weight": int(grammar_weight), "content_weight": int(content_weight), "active": True,
                    }).execute()
                    st.success("Question added."); st.rerun()
                except Exception as e: st.error(str(e))
        try:
            interview_questions = supabase.table("ai_interview_questions").select("*").order("id").execute().data or []
            for q in interview_questions:
                c1, c2 = st.columns([6, 1])
                with c1:
                    st.write(q.get("question"))
                    st.caption(f"{q.get('time_seconds', 60)} sec • Max {q.get('max_marks', 10)} marks • {'AI evaluation ON' if q.get('ai_evaluate') else 'Manual review'}")
                with c2:
                    if st.button("Delete", key=f"del_interview_q_{q['id']}"):
                        supabase.table("ai_interview_questions").delete().eq("id", q["id"]).execute(); st.rerun()
            responses = supabase.table("ai_interview_responses").select("*, users(name), ai_interview_questions(question)").order("created_at", desc=True).limit(100).execute().data or []
            st.markdown("#### Recorded responses")
            for r in responses:
                st.write(f"**{(r.get('users') or {}).get('name', 'Student')}** — {(r.get('ai_interview_questions') or {}).get('question', '')}")
                st.caption(f"Score: {r.get('score') if r.get('score') is not None else 'Pending'} | {r.get('evaluation', '')}")
                if r.get("audio_path"):
                    url = supabase.storage.from_("interview-recordings").create_signed_url(r["audio_path"], 3600).get("signedURL")
                    st.audio(url)
        except Exception as e: st.info(f"Interview data unavailable until setup is completed: {e}")
    with tabs[1]:
        enabled = feedback_collection_enabled()
        st.write("Collection status: **Open**" if enabled else "Collection status: **Closed**")
        if st.button("Close Feedback Collection" if enabled else "Send to Collect Feedback", type="primary"):
            try:
                supabase.table("feedback_settings").upsert({"id": 1, "collection_enabled": not enabled}).execute()
                st.rerun()
            except Exception as e: st.error(str(e))
        try:
            feedback_rows = supabase.table("user_feedback").select("*, users(name, email)").order("created_at", desc=True).limit(200).execute().data or []
            for item in feedback_rows:
                user = item.get("users") or {}
                with st.container(border=True):
                    st.markdown(f"**{user.get('name', 'Student')}** ({user.get('email', '')})")
                    st.write(item.get("feedback", "")); st.caption(str(item.get("created_at", "")))
        except Exception as e: st.info(f"No feedback data yet: {e}")
    with tabs[2]:
        with st.form("add_study_note", clear_on_submit=True):
            topic = st.text_input("Topic / Folder name")
            question = st.text_area("Question")
            answer = st.text_area("Answer")
            source = st.text_input("Source link (optional)")
            if st.form_submit_button("Add Note", type="primary"):
                if all([topic.strip(), question.strip(), answer.strip()]):
                    try:
                        supabase.table("study_notes").insert({"topic": topic.strip(), "question": question.strip(), "answer": answer.strip(), "source_url": source.strip() or None}).execute()
                        st.success("Note added.")
                    except Exception as e: st.error(str(e))
                else: st.warning("Topic, question and answer required.")
...
def user_dashboard(preview_mode=False):
    ...
    if not preview_mode:
        ...
        if st.sidebar.button("Logout", use_container_width=True):
            ...
            show_logout_redirect()
        st.sidebar.divider()
        pages = ["My Classes", "Exams", "AI Mock Interview", "Notes", "Feedback", "Progress", "Code Practice", "Group Chat", "Attendance"]
    ...
    if user_page == "AI Mock Interview":
        show_ai_mock_interview(st.session_state.user_id); return
    if user_page == "Notes":
    ...
    for module in modules:
        ...
        with st.expander(f"{module['title']}    {module_done}/{module_total} classes  ({pct}%)", expanded=module_has_focus):
            ...
            for sub in submodules:
                ...
                # Each submodule is a folder, so a long class list stays collapsed until needed.
                with st.expander(f"📁 {sub['title']}   {sub_done}/{sub_total}", expanded=bool(focus_class_id and any(str(c.get('id')) == focus_class_id for c in sub_classes))):
